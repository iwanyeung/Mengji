...
1. 手表可连录多段，每段传到手机后进入录梦 Tab 草稿列表（来源「手表」），**不**自动梦析。
2. 手机可在同一场梦中继续录段；勾选要参与的草稿后点「完成并整理」。
3. 整理成功 / 四格落成：手表 `WCSession` 通知 + 轻微震动；服务端 `dream_analyzed` / `visual_done` APNs 作兜底（需配置 `APNS_*`）。
